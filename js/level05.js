...
function resetGame(){
  game = {speed:0,
          initSpeed:.00003,
          baseSpeed:.00003,
          targetBaseSpeed:.00003,
          incrementSpeedByTime:.0000023,
          incrementSpeedByLevel:.0000023,
          ...
          planeDefaultHeight:350,
          planeAmpHeight:175,
          planeAmpWidth:220,
          planeMoveSensivity:0.0061,
          ...
          cameraFarPos:590,
          cameraNearPos:580,
          cameraSensivity:0.000019,

          coinDistanceTolerance:18,
          coinValue:6,
          coinsSpeed:.4,
          coinLastSpawn:0,
...
function createScene() {
  ...
  camera = new THREE.PerspectiveCamera(
    ...
    scene.fog = new THREE.Fog(0xFC9E4F, 200,1700);
      //aduis
...
function createLights() {
  ...
  ambientLight = new THREE.AmbientLight(0x7D8EA2, .5);
...
/**rayo */
// Point Light
flash = new THREE.PointLight(0xcdbfbc, 0, 0 ,-1.7);
...
Sky = function(){
  ...
  for(var i=0; i<this.nClouds; i++){
    ...
    c.mesh.position.z = -500-Math.random()*300;
...
function sun(){
    ...
    skySun.scale.setScalar( 45000);
    skySun.material.uniforms.turbidity.value =200;//1
    skySun.material.uniforms.rayleigh.value = -0.115; //menos es mas oscuro 2:noche, 1:tarde
    skySun.material.uniforms.luminance.value = -0.001;//tarde:0.001
    skySun.material.uniforms.mieCoefficient.value = 0.105//0.1:esfera mas iluminada sol
    skySun.material.uniforms.mieDirectionalG.value = 0.1895;
    ...
    sunSphere.position.y = -100001 * Math.sin( phi ) * Math.sin( theta );//posicion de la esfera
...
Forest = function(){
  ...
  for(var i=0; i<this.nClouds; i++){
    ...
    this.clouds.push(c);
    var a = stepAngle*i;
    var h = game.seaRadius - 14 + Math.random()*420;
    //c.mesh.position.y = Math.sin(a)*h;
    c.mesh.position.y = Math.sin(a)*h;
    c.mesh.position.x = Math.cos(a)*h;
...
var particles = new THREE.ParticleSystem( geometry, materials.cloud );
...
*/
    ...
    if(skySun.material.uniforms.turbidity.value<-10)
    {
        ...
        if(lowerMaxFr>0)
    skySun.material.uniforms.turbidity.value-=0.1*(Math.random()*(lowerMaxFr/9));
    }
    if(skySun.material.uniforms.rayleigh.value<-10)
    {
        ...
        if(lowerMaxFr>0)
        skySun.material.uniforms.rayleigh.value-=0.1*(Math.random()*(lowerMaxFr/9));
    }
   ...
   /*
    ...
    var phi = 2 * Math.PI * ( -.25 );
    if(upperAvgFr>0)
    sunSphere.position.y-=80000 * Math.sin( phi ) * Math.sin( theta )+(upperAvgFr*10000);
   */
  ...
  */
  ...
  if(Math.random() > 0.96 || flash.power > 100) {
    if(flash.power < 100) 
      ...
      //if(lowerMaxFr>1)
    flash.power = 50 + Math.random() * 500 * lowerMaxFr;
    //else
...
function createMountainsBack(){
   ...
   loader.load("img/mountain1.png", function(texture) {
     cloudGeo = new THREE.PlaneBufferGeometry(4000,2000);
