...
bgmusic = new Howl({
  src: ['sounds/carnavalito-lito.ogg'],
  loop: true,
  volume: 0.5,
  pause:true
});
...
function resetGame(){
  game = {speed:0,
          ...
          planeDefaultHeight:270,
          planeAmpHeight:175,
          planeAmpWidth:120,
          planeMoveSensivity:0.0031,
...
function createScene() {
  ...
  scene.fog = new THREE.Fog(0xf3b05a, 200,1800);
  //aduis
...
function createLights() {
  ...
  ambientLight = new THREE.AmbientLight(0xFFBF00, .5);
...
function sun(){
    ...
    skySun.material.uniforms.turbidity.value =30;//1
    skySun.material.uniforms.rayleigh.value = 3.11; //menos es mas oscuro 2:noche, 1:tarde
    skySun.material.uniforms.luminance.value = 0.111;//tarde:0.001
    skySun.material.uniforms.mieCoefficient.value = 0.0013//0.1:esfera mas iluminada sol
    skySun.material.uniforms.mieDirectionalG.value = 0.55;
...
var Cactus=function(){
  ...
  stem.scale.set(0.08,0.08,0.08);
	...
	petalCore.position.y += 0.2;
  //petalCore.position.z = 3;
  petalCore.scale.set(0.009,0.009,0.009);
...
Forest = function(){
  ...
  for(var i=0; i<this.nClouds; i++){
    ...
    this.clouds.push(c);
    var a = stepAngle*i;
    var h = game.seaRadius - 1 + Math.random()*6;
    //c.mesh.position.y = Math.sin(a)*h;
    c.mesh.position.y = Math.sin(a)*h;
    c.mesh.position.x = Math.cos(a)*h;
    c.mesh.position.z = 310-Math.random()*500;
    c.mesh.rotation.z = a - Math.PI/2;
