...
function resetGame(){
  game = {speed:0,
          initSpeed:.00008,
          baseSpeed:.00008,
          targetBaseSpeed:.00008,
          incrementSpeedByTime:.000003,
          ...
          level:4,
          levelLastUpdate:0,
          distanceForLevelUpdate:350,

          planeDefaultHeight:250,
          planeAmpHeight:175,
          planeAmpWidth:120,
          planeMoveSensivity:0.0041,
          planeRotXSensivity:0.0004,
          planeRotZSensivity:0.0002,
          planeFallSpeed:.001,
          planeMinSpeed:1.2,
          planeMaxSpeed:1.8,
          planeSpeed:0,
...
function createScene() {
  ...
  camera = new THREE.PerspectiveCamera(
    ...
    scene.fog = new THREE.Fog(0x747880, 200,1500);
      //aduis
...
function createLights() {
  ...
  ambientLight = new THREE.AmbientLight(0xBDD09F, .5);
...
/**rayo */
// Point Light
flash = new THREE.PointLight(0x83929f, 0, 0 ,-1.7);
flash.position.set(5,5,1);
scene.add(flash);
  //scene.add(ch);
...
Sky = function(){
  ...
  for(var i=0; i<this.nClouds; i++){
    ...
    c.mesh.position.z = -10-Math.random()*900;
...
Cloud = function(){
  ...
  var geom = new THREE.TetrahedronGeometry(12.5,2);
...
Ennemy = function(){
  ...
  var geom = new THREE.TetrahedronGeometry(15,3);
...
EnnemiesHolder.prototype.rotateEnnemies = function(min,max){
  for (var i=0; i<this.ennemiesInUse.length; i++){
    ...
    ennemy.mesh.position.y = -game.seaRadius + Math.sin(ennemy.angle)*ennemy.distance+80;
...
function sun(){
    ...
    skySun.scale.setScalar( 95000);
    skySun.material.uniforms.turbidity.value =50;//1
    skySun.material.uniforms.rayleigh.value = 3.5; //menos es mas oscuro 2:noche, 1:tarde
    skySun.material.uniforms.luminance.value = 1.011;//tarde:0.001
    skySun.material.uniforms.mieCoefficient.value = 3.013//0.1:esfera mas iluminada sol
    skySun.material.uniforms.mieDirectionalG.value = 0.895;
    ...
    sunSphere.position.x = 4000 * Math.cos( phi );
    sunSphere.position.y = 80500 * Math.sin( phi ) * Math.sin( theta );//posicion de la esfera
    sunSphere.position.z = 4000* Math.sin( phi ) * Math.cos( theta );
  ...
  rainCount = 15000;
   ...
   /**
    ...
    // Create a rain drop for each particle
...
var Cactus=function(){
  ...
  var material = new THREE.MeshNormalMaterial( { color: Colors.brown } );
  ...
  var material = new THREE.MeshLambertMaterial( { color: 0x0CA4A5 } );
  ...
  //this.mesh.rotate.set(0,0,Math.PI/2);
...
Forest = function(){
  ...
  for(var i=0; i<this.nClouds; i++){
    ...
    this.clouds.push(c);
    var a = stepAngle*i;
    var h = game.seaRadius - 14 + Math.random()*12;
    //c.mesh.position.y = Math.sin(a)*h;
    c.mesh.position.y = Math.sin(a)*h;
    c.mesh.position.x = Math.cos(a)*h;
    c.mesh.position.z = -5-Math.random()*200;
    c.mesh.rotation.z = a - Math.PI/2;
    var s = 15+Math.random()*35;
    c.mesh.scale.set(s,s,s);
    this.mesh.add(c.mesh);
  }
}
...
//makeRoughGround(sea.mesh, modulate(upperMaxFr, 0, 1, 0.5, 4));
    ...
    */
    ...
    updateEnergy();
    var mouseSpeed=1;
    var calculoSpeed=(mousePos.x / WIDTH)*1500;
    if(calculoSpeed>1)
...
*/
   ...
   /*
    ...
    var phi = 2 * Math.PI * ( -.25 );
    if(upperAvgFr>0)
    sunSphere.position.y+=405000 * Math.sin( phi ) * Math.sin( theta )+(upperAvgFr*100);
   */
...
/**
  ...
  rainGeo.vertices.forEach(p => {
    p.velocity -= 0.12 + Math.random() * 0.14;
    p.y += p.velocity;
    if (p.y < -500) {
      p.y = 2500;
  ...
  if(Math.random() > 0.96 || flash.power > 75) {
    if(flash.power < 50) 
      flash.position.set(
        Math.random()*200,
        150 + Math.random() *100,
      ...
      //if(lowerMaxFr>1)
    flash.power = 15 + Math.random() * 150 * lowerMaxFr;
    //else
...
function updateDistance(){
  ...
  distancia_final.innerHTML = 'Miles: ' + Math.floor(game.distance);
...
function createMountainsBack(){
 ...
 loader.load("img/mountain2.png", function(texture) {
   cloudGeo = new THREE.PlaneBufferGeometry(6000,1500);
...
function updatePlane(min,max){
  ...
  if(min>0 )
airplane.mesh.scale.set(1.1+(min/2),1.1+(min/2),1.1+(min/2));
  else
  airplane.mesh.scale.set(1.65,1.65,1.65);
